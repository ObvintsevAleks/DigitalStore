...
public class InvoiceService {
    ...
    public InvoiceDTO getInvoiceById(UUID id) throws NotFoundInDBException {
        Invoice invoice = invoiceRepository.findById(id).orElseThrow(() -> new NotFoundInDBException("Не найден инвойс по id = "+ id));
        return invoiceMapper.toInvoiceDTO(invoice);
    }

    @Transactional
    public InvoiceDTO updateInvoice(InvoiceDTO invoiceDTO) throws NotFoundInDBException {
        Invoice invoiceEntity = invoiceRepository.findById(invoiceDTO.getId()).orElseThrow(() -> new NotFoundInDBException("Не найден инвойс по id = "+ invoiceDTO.getId()));
